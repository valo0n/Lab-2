...
export default function AdminBrands() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || "Nuk u ngarkuan brandet.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="m-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-bold">
                {editId ? "Edito brandin" : "Brand i ri"}
              </h4>
              ...
              <tbody>
                {items.map((b) => (
                  <tr key={b.id} className="border-t border-gray-50">
                    ...
                    <td className="px-5 py-3 text-dark-300">{b.slug}</td>
                    <td className="px-5 py-3">
                      <span
                        className={`text-xs font-bold uppercase ${b.is_active ? "text-green-600" : "text-red-600"}`}
                      >
